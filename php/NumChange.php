...
//		  $useId = 1;
		...
		//  $useId = 1;
		    ...
		    $sql_check = "SELECT id from 积分购物车  where `积分商品id` = '".$excId."' and `用户信息id` = '".$useId."' and '是否付款' = 0";
		    $result = $conn->query($sql_check)->fetch_assoc();
		    //需要新建数据信息
		    if(!(isset($result['id']))){
		        $sql_insert = "insert into 积分购物车 (积分商品id,用户信息id,商品数量,下单时间,是否付款) values('".$excId."','".$useId."','".$num."','".$time."','0')";
		        $conn->query($sql_insert);
		    }
		    //存在数据信息
		    else{
		        $sql_update = "update 积分购物车 set 商品数量 = '".$num."' where id = '".$result['id']."' and 是否付款 = 0 ";
		        $conn->query($sql_update);
		    }
		break;
	    default:break;
